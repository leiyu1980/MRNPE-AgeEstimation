...
cv::Mat sim_transform_image_3channels(cv::Mat img, int t_width, int t_height, float eye_cx,
									  float eye_cy, float mx, float my, double target_eye_x, double target_mouth_x,
									  double target_eye_y, double target_mouth_y)
{
	float a1,b1,tx1,ty1;
	double tempx;
	double tempy;
...
//	TPointF dstPoints[2] = {
//			{t_width >> 1, target_eye_y},
//			{t_width >> 1, target_mouth_y}
//	};
	TPointF dstPoints[2] = {
			{target_eye_x, target_eye_y},
			{target_mouth_x , target_mouth_y}
	};
	TPointF srcPoints[2] = {
			{eye_cx, eye_cy},
			{mx, my}};
...
//	circle(img_crop,Point(dstPoints[0].x,dstPoints[0].y),1,Scalar(0,255,0));
//	circle(img_crop,Point(dstPoints[1].x,dstPoints[1].y),1,Scalar(0,255,0));
//	resize(img_crop,img_crop,Size(480,400));
//	imshow("test_image3",img_crop);
//	waitKey();
